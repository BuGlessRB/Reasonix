...
// whether anything actually established it. The order is most-specific first:
// what the config says for this provider (including its per-model overrides),
// then what is known about the model itself. Callers that only need the number
// can read entry.ContextWindow; callers that must not confuse "nobody said"
// with "no compaction wanted" need the second return.
