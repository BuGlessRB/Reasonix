...
func TestGPT56MinimalDegradesInsteadOfBeingSent(t *testing.T) {
	...
	for _, lvl := range []string{"none", "xhigh", "max"} {
		if v, err := NormalizeEffort(relay, lvl); err != nil || v != lvl {
			t.Fatalf("NormalizeEffort(%q) = %q, %v — want it accepted as itself", lvl, v, err)
		}
	}
