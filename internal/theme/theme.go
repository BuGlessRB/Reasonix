...
// List returns every readable pack, sorted by id. A missing directory is not
// an error — it means nothing is installed. An unreadable or malformed pack is
// skipped so one bad file cannot hide the rest.
func List() []Pack {
...
func Load(id string) (Pack, error) {
	...
	return load(filepath.Join(Dir(), id, manifestName), id)
}
