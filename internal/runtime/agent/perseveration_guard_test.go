...
// retriesPtr makes the optional Options.MaxPerseverationRetries explicit in a
// table without a package-level helper shared with other suites.
...
func TestStreamAbortsOnPerseverationLoop(t *testing.T) {
	...
	a := New(prov, tool.NewRegistry(), sessionstore.NewSession(""), Options{ModelRef: "loop/model"}, sink)
...
func TestStreamDoesNotAbortOnLongNormalAnswer(t *testing.T) {
	...
	a := New(prov, tool.NewRegistry(), sessionstore.NewSession(""), Options{ModelRef: "normal/model"}, sink)
...
func TestPerseverationRetryBudgetIsConfigurable(t *testing.T) {
	cases := []struct {
		...
		{"zero disables the retry", retriesPtr(0), 1},
		{"unset keeps the default", nil, 2},
		{"two retries", retriesPtr(2), 3},
	}
...
func TestStreamAbortsOnReasoningLoop(t *testing.T) {
	...
	sink := event.FuncSink(func(event.Event) {})
	a := New(prov, tool.NewRegistry(), sessionstore.NewSession(""), Options{ModelRef: "think-loop/model"}, sink)
...
func TestStreamAbortsOnReasoningLoopInterleavedWithAnswer(t *testing.T) {
	...
	sink := event.FuncSink(func(event.Event) {})
	a := New(prov, tool.NewRegistry(), sessionstore.NewSession(""), Options{ModelRef: "mixed-loop/model"}, sink)
