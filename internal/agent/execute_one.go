...
func (a *Agent) applyPlanModeAndProxy(ctx context.Context, plan *toolCallPlan) (toolOutcome, bool) {
	...
	if resolver, ok := t.(tool.CallResolver); ok {
		...
		if rc.SkipExecute {
			...
			body, truncMsg := a.boundToolOutput(result, plan.call.Name, plan.call.ID, plan.call.Arguments)
...
func (a *Agent) finishToolExecution(ctx context.Context, plan *toolCallPlan) toolOutcome {
	...
	if err != nil {
		...
		body, truncMsg := a.boundToolOutput(rawErr, call.Name, call.ID, call.Arguments)
		out := toolOutcome{
	...
	body, truncMsg := a.boundToolOutput(result, call.Name, call.ID, call.Arguments)
	out := toolOutcome{
