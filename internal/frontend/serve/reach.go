...
func withDeviceReach(ctx context.Context, deviceID string) context.Context {
	return context.WithValue(ctx, deviceReachKey{}, deviceID)
}

// DeviceOf names the paired device a request came from, if it came from one.
func DeviceOf(ctx context.Context) (string, bool) {
	id, ok := ctx.Value(deviceReachKey{}).(string)
	return id, ok
}
