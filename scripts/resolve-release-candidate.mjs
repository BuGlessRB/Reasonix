...
export function selectRecordArtifact(artifacts, candidateId, required = true) {
  if (!ID_RE.test(candidateId)) throw new Error("invalid release candidate id");
  const name = `release-candidate-record-${candidateId}`;
...
export function inspectRecord(record, candidateId, recordArtifact, run, now = new Date()) {
  ...
  if (record.source?.payloadArtifactName !== `release-candidate-payload-${candidateId}`) throw new Error("candidate payload artifact name mismatch");
  if (!/^[1-9][0-9]*$/.test(record.source?.evidenceArtifactId ?? "")) throw new Error("candidate evidence artifact id is invalid");
  if (record.source?.evidenceArtifactName !== `release-candidate-evidence-${candidateId}`) throw new Error("candidate evidence artifact name mismatch");
...
async function resolve(candidateId, required = true) {
  ...
  const artifactData = await githubJSON(`/repos/${repository}/actions/artifacts?name=${encodeURIComponent(`release-candidate-record-${candidateId}`)}&per_page=100`, token);
  const artifact = selectRecordArtifact(artifactData.artifacts ?? [], candidateId, required);
  if (!artifact) {
    outputs({ found: false });
    return;
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  } else throw new Error("usage: resolve-release-candidate.mjs active ID | resolve ID | resolve-optional ID | inspect ID RECORD ARTIFACT RUN");
}
