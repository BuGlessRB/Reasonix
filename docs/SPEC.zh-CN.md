...
| 依赖的最终答案 | 首个 user turn 上的 `<upstream-results>`，仅限声明了 `depends_on` 的 fleet item（见 §3.14） |

按设计**不继承**：`REASONIX.md`、`AGENTS.md`、`CLAUDE.md`、项目与全局记忆（memory queue 被关闭，子智能体也无法写入记忆）、父对话、当前 Goal、planner 输出，以及它**没有声明依赖**的那些同级子智能体的结果。今天要让一条约束抵达子智能体，只能写进它的 profile body、任务文本，或一条 `depends_on` 边——不存在环境通道。

每次运行都会在其 transcript sidecar 中记录一份 `ContextCapsule`：workspace、系统提示来源与哈希、解析后的工具范围与 schema 哈希、model 与 effort、父会话与父工具调用 id、续接的 transcript，以及一个 `inherited` 块——其中各字段均为 false，唯一的例外是被 `depends_on` 边喂养过的条目上的 `upstream`。`capsuleHash` 是它的稳定标识，因此"为什么这个 reviewer 没看到那条约束"可以从记录回答，两次行为不同的运行也可以直接比对而不是猜。capsule 只保存引用与摘要——绝不复制父上下文，这正是委派保持低成本、子前缀保持可缓存的原因。
...
依赖是图的性质，不是任务的性质：它只存在于 fleet plan 中，绝不进入 `TaskSpec`。这正是让 `depends_on` 不至于成为某种 workflow 语言第一个关键字的原因。

边携带的是依赖的答案，不只是它的次序。依赖项一开场就拿到每个已完成依赖的最终答案，装在 `<upstream-results>` 块里，按 `depends_on` 的声明顺序排列，并共享同一份字节预算——因此宽 fan-in 无法淹没开场 turn，也没有哪个依赖会被挤出预算之外。一条只排序、不交付任何东西的边，会让 `research → implement` 这一对为同一份 research 付两次钱。边交付的是上下文而不是指令，所以它走 `ContextRequest.Upstream`——绝不塞进任务文本（那必须保持交付分类所评判的那份原始目标，见 §3.11），也绝不挂在 profile 上（profile 描述的是一个 worker，不是一次运行）。
...
失败携带的是身份，不只是一句话。失败的条目在状态旁报出 `reason=`——`provider.transient` 表示一次后续尝试有可能通过，`provider.rejected` 表示重试帮不上忙，`context.exhausted` 表示输入超出了窗口且压缩已无力回天——而被它杀掉的整条分支继承同一个 reason，因为一个被跳过的条目值不值得重发，恰好取决于它的起因值不值得重发。分类只读错误本身已经携带的身份（`*provider.APIError` 的状态码、`ErrCompactionRequired`、被中断的流），绝不读它的消息文本，所以一个生产者没有命名的错误会被如实报成 unclassified，而不是被猜出一个类别。于是「把这张图带着已完成的节点重发一遍」这个决定，读的是一个代码，而不是一句话。

一次跑到终点的运行不返回错误，无论其中失败了多少条目：失败由聚合报告，标题给出有多少个任务给出了答案。`context.Canceled` 只留给真正被打断的调用——那正是 turn loop 用来区分「被打断」与「跑完并且报告了」的东西。一条因上游失败而被跳过的分支，是那次失败的后果，不是一次打断；把它说成打断，代价由调用方承担。

一个节点可以让位给一条引用，而不是去运行。`adopt_ref` 指名一个已完成的子智能体，由它的答案顶替该节点：什么都不执行，所以该条目不持有任何 `write_paths`，不可能与活跃写入者相撞；它的依赖项照常通过普通依赖边拿到那份答案。它在 preflight 过的是与 `read_subagent_result` 完全相同的那道门——已完成、且位于本对话的血缘与 workspace 之内——因此一条无法被采纳的引用不会启动任何东西，而 `prompt` 与所有执行字段在它上面会被明确拒绝，而不是被静默忽略。

宿主不会做的那件事，是判断一个被采纳的答案是否仍然有效。结构里没有任何东西能说明 workspace 在它之下是否已经变了，而这个内核读不出结构的判断，它就不做——那一项留给列出这条引用的调用方，宿主只证明归属与完成。这也是为什么 plan 指纹不会静默复用匹配上的子智能体：调用方看不见的复用，就是它纠正不了的复用。配合 `list_subagents`，这就是一张被重启打断的 fleet 的恢复路径——枚举子智能体，把已完成的节点改写成采纳后重发同一张图，只为剩下的部分付钱。

“没有动态扩散”是被测量排除的，不只是一种口味。一个把某条目的 prompt 映射到另一条目所提交的若干 subject 上的 `for_each` 曾被完整实现，并在不进入 provider 可见 schema 的前提下上线，随后与它所替代的那次调用做了对照——同一张图，由一个任务处理全部 subject，那正是没有它时必须写成的样子。三个 fixture、二十八次针对真实 provider 的评分运行，两臂每一次都解出来了，而映射为此多花了 70% 到 98% 的 token（各自的精确置换检验 p ≤ 0.03）。第三个 fixture 是专门造出来让映射赢的：十个近乎相同的模块，答案是十次独立推导之和，任何一处闪失都是错答案。它在那里也没有赢。

原因在于：一个被映射出来的子智能体，每个 subject 都要付一次完整子智能体的固定成本——前缀、推理、收尾契约——而一个带着工具带的单个子智能体，根本不必去“持有”这些 subject。它一次读一个，当事实是可提取的，它就直接 grep。“一个 context 装不下 N 个 subject”这句话，预设了一次智能体其实从来不必执行的 ingestion。相关代码与其测试套件已被移除；如果哪天真出现需要按 subject 隔离的案例，历史里还留着它们。
