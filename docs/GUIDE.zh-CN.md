...
[配置路径](./CONFIG_PATHS.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被项目本地配置覆盖。
...
主机保存在 `config.toml` 的用户级 `[remote]` 段。与 `[secrets]` 一样,项目本地配置
无法注入或覆盖远程主机 —— 克隆的仓库永远无法左右 Reasonix 向何处发起 SSH
