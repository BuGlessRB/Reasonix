...
- 桌面端点击输入框左下角菜单，在“执行设定”下选择“均衡”或“交付”。
- CLI 启动时使用 `reasonix --preset balanced|delivery`，非交互运行使用
  `reasonix run --preset ...`。兼容 `--profile economy|balanced|delivery`。
- TUI 会话内使用 `/preset balanced|delivery` 热切换；不带参数的 `/preset` 会列出
