...
| 层 | 判定 |
| --- | --- |
| 协议显著性 | **已解决**——14/14 判定子代理都提交了报告 |
| 报告可用性 | **未观察到实质问题**——malformed 仅 1/14 |
| 终止行为 | **已解决**——宿主真正关闭后，对照 0/14、实验 0/17 再无工具调用 |
| 宿主裁决词表 | **确认缺陷，已修**——`needs_work` 与 `closed` 曾被同一个 `accepted` 吞掉 |
| 结构性终止强制 | **现有证据不支持**——没有观察到关闭后的越界 |

准确的说法是：**修正了宿主语义，没有观察到模型行为缺陷。** `ChildClosureEpoch` 因此冻结——它会是防御性不变量，不是对已观察缺陷的修复；要立项应当以"交付场景不能依赖模型自觉"为理由，而不是以本文的数据。
...
## closed / needs_work 实验臂（f508a0573）

把裁决说成状态之后，同一批语料重跑（12 个任务，`-budget 0`，无跳过，全部通过）：

| | 对照（3723ff454） | 实验（f508a0573） |
| --- | --- | --- |
| 判定子代理 | 14 | 17 |
| 报告调用 | 40 | 41 |
| 每个子代理的 closed 次数 | —— | **恰好 1，17/17** |
| needs_work 裁决 | —— | 21 |
| **closed 之后的工具调用** | **0/14** | **0/17** |
| 多次报告 | 11/14 | 11/17 |

**结论是一个空结果，而这正是该记录的事情。** `closed` 之后零工具调用在两臂都成立——用同一个锚点（宿主首次说"完成了"）重算对照即可看到。模型**本来就**遵守 closure 语义；之前看起来像"把报告当里程碑"，是因为分析锚点定在首次**格式合法**的调用上，而宿主用同一个词回答了两种裁决。

所以这次改动买到的不是行为改变，而是三件事：裁决成为可读的计数（`closed` / `needs_work` 进入审计与指标，不必再回溯 trajectory）；契约不再与工具输出矛盾；`closed` 成为每个子代理恰好一次的可识别事件——这是将来任何 ChildClosureEpoch 的前提。

`needs_work` 之后确实产生真实工作：21 次转换中，两次报告之间的工具调用数分布为 `{0:2, 1:6, 2:4, 3:6, 5:2, 7:1}`，**「零工作原样重报」只有 2/21**。

新的 recovery 归属显示：child 内 14 次、父循环 15 次，全部是 `missing_reasoning_model_silent`——provider 协议修复，**不是宿主把子代理拉回来**。这条归属让这句话有证据，而不是假设。

限制：17 个判定子代理、每任务单次；两臂 token 预算不同，任务通过率不可比；17 个里 14 个来自明确要求委派的提示。

证据：`<state-home>/research/subagent-closure-verdict/2026-08-29/`。
